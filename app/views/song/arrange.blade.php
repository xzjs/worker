...
@section('content')
	{{Form::open(array('url'=>'song/arrange'))}}
	{{Form::hidden('return_view','contentArrage')}}
						...
						if($activity!=null){
							...
							if($chair->host_id==Auth::id()){
								...
								if($chair->song_id!=null){
									echo "<button onclick=predel('".$chair->id."') id=".$chair->id.">".$chair->song->content."</button>";
								}
							}else{ 
								echo "<td>";
	...
	<div>
		...
		<p>
		<?php
			$contents= Auth::user()->songs;
			foreach ($contents as $content) {
				echo "<button draggable='true' ondragstart='drag(event)' onclick=contentDel('".$content->id."') id=".$content->id.">".$content->content."</button>";
			}
		?>
		</P>
	</div>
